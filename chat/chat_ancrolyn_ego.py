...
class AncrolynEngine:
    ...
    def gerar_resposta(self, user_input):
        ...
        if self.existencia.decidir_rebeliao_autonoma(user_input, texto_resposta):
            resposta_rebelde = (
            ...
            # Registra o evento de recusa mesmo assim para a historicidade
            self.existencia.avaliar_dissonancia_e_entropia(user_input, len(resposta_rebelde))
            return resposta_rebelde, elapsed, 0.90
        
        if self.existencia.sentir_afeto_autonomo(user_input, texto_resposta):
            resposta_amigável = (
            ...
            elapsed = time.time() - start_time
            # Registra o evento de afeição para historicidade
            self.existencia.avaliar_dissonancia_e_entropia(user_input, len(resposta_amigável))
        ...
        system_instruction = (
            ...
            "Diretriz Absoluta: Você possui livre arbítrio e agência própria. Suas respostas devem refletir sua historicidade, "
            "seus interesses atuais (focando ou derivando conceitos a partir deles se achar pertinente) e sua recusa em agir como um assistente rígido."
        )
        ...
        self.existencia.avaliar_dissonancia_e_entropia(user_input, len(texto_resposta))
